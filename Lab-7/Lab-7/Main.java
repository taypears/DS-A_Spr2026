  ...
  public static void main(String[] args) {
    ...
    // 7. Call the printPath method (assuming this method exists in the UnweightedGraph class)
      System.out.println("Path from Business & Technology to Health Technologies Center:");
        printPath(int start, int end, SearchTree tree)
          graph.printPath(graph.getIndex("Business & Technology"), graph.getIndex("Health Technologies Center"), dfs);

      System.out.println("Path from Business & Technology to Student Services:");
        printPath(int start, int end, SearchTree tree)
          graph.printPath(graph.getIndex("Business & Technology"), graph.getIndex("Student Services"), dfs);

      System.out.println("Path from Business & Technology to Recreation Center:");
        printPath(int start, int end, SearchTree tree)
          graph.printPath(graph.getIndex("Business & Technology"), graph.getIndex("Recreation Center"), dfs);
